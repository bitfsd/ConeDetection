...
class PPinfer:
    ...
    def __init__(self):
        ...
        self.scale_factor = np.array(
            [self.im_size * 1. / img.shape[0], self.im_size * 1. / img.shape[1]]).reshape((1, 2)).astype(np.float32)
        self.im_shape = np.array([self.im_size, self.im_size]).reshape(
    ...
    def subscribe(self):
        rospy.Subscriber(
            self.Subscriber['camera_topic_name'], Image, self.predict_img)

    def publish(self):
        self.image_pub = rospy.Publisher(
            self.Publisher['detected_image_topic_name'], Image, queue_size=1)  # 定义话题
        self.boundingbox_pub = rospy.Publisher(
            self.Publisher['boundingbox_topic_name'], BoundingBoxes, queue_size=1)
        self.object_pub = rospy.Publisher(
            self.Publisher['object_topic_name'], ObjectCount, queue_size=1)
    ...
    def predict_config(self, model_file, params_file):
        ...
        if self.Parameters['use_tensorrt']:
            if self.Parameters['tensorrt_type'] == 'FP16':
                config.enable_tensorrt_engine(workspace_size=1 << 30, precision_mode=PrecisionType.Half,
                                              max_batch_size=1, min_subgraph_size=5, use_static=True, use_calib_mode=False)
            if self.Parameters['tensorrt_type'] == 'FP32':
                config.enable_tensorrt_engine(workspace_size=1 << 30, precision_mode=PrecisionType.Float32,
                                              max_batch_size=1, min_subgraph_size=5, use_static=True, use_calib_mode=False)
            if self.Parameters['tensorrt_type'] == 'INT8':
                config.enable_tensorrt_engine(workspace_size=1 << 30, precision_mode=PrecisionType.Int8,
                                              max_batch_size=1, min_subgraph_size=5, use_static=True, use_calib_mode=True)
    ...
    def draw_bbox(self, frame, result, label_list, shape, threshold=0.5):
        ...
        for res in result:
            ...
            cv2.rectangle(frame, (int(xmin), int(ymin)),
                          (int(xmax), int(ymax)), (255, 0, 255), 2)
            label_id = label_list[int(cat_id)]
            print('label is {}, bbox is {}'.format(
                label_id, [int(xmin), int(xmax), int(ymin), int(ymax)]))
            try:
                cv2.putText(frame, label_id, (int(xmin+10), int(ymin-2)),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5 / self.im_size * height, (255, 255, 255), 2)
                cv2.putText(frame, str(round(score, 2)), (int(xmin-35), int(ymin-2)),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5 / self.im_size * height, (0, 255, 0), 2)
            except KeyError:
                pass
    ...
    def predict_img(self, msg):
        ...
        t1 = time.time()
        result = self.predict(
            self.predictor, [self.im_shape, img_data, self.scale_factor])
        t2 = time.time()
        os.system("clear")
        ...
        threshold = 0.5
        self.draw_bbox(img, result[0], self.label_list,
                       cv_img.shape, threshold=threshold)
        header = Header(stamp=rospy.Time.now())
        header.frame_id = "Camera"
